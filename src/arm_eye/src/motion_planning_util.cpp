...
vector<double> ur5e_action::GetJoints(string name) {
  ...
  if (_Inst_joints.find(name) != _Inst_joints.end()) {
    ...
    cout << "GetJoints=" << name << endl;
    ROS_ERROR_STREAM("no_joints_value");
...
void ur5e_action::pgi_Callback(
    const std_msgs::StringConstPtr &pgiOperation_in) {
  try {
    Json::Reader jsonreader;
    Json::Value root;
    jsonreader.parse(pgiOperation_in->data, root);
    uint16_t grip_length = (uint16_t)root["operation"].asInt();
    grip_length = grip_length > 1000 ? 1000 : grip_length;
    grip_length = grip_length < 0 ? 0 : grip_length;
    Gripper_op(grip_length);
  } catch (std::exception e) {
    ROS_ERROR_STREAM("Set pgi pose failed,error message:");
    ...
    pubmsgs.data = pub_data.toStyledString();
