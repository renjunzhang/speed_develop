...
int dashboardsrv_client::load_program(const string filename)
{
  ...
  if (!programState->data)
  {
    ...
    while (!ros::topic::waitForMessage<std_msgs::Bool>("/ur_hardware_interface/robot_program_running")->data)
    {
      ...
      count++;
      if(count > 5){
        ROS_ERROR_STREAM("Robot load program failed after waiting for 5 second\n");
        return -1;
      }
    }
  }
...
int dashboardsrv_client::robot_init()
{
  ...
  while (robotmode->mode != robotmode->RUNNING)
  {
    ...
    count++;
    if(count > 20){
        ROS_ERROR_STREAM("Robot init failed after waiting for 20 second\n");
        return -1;
    }
  }
  ROS_INFO_STREAM("Robot is RUNNING\n");
