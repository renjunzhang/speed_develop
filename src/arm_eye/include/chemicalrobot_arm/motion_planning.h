...
private:
  ...
  /**
   * @brief
   * _sub1_用于接受操作指令，_sub2_用于监测机械臂safety_mode,_sub3_用于监测轨迹执行状态,_sub4_用于监听夹爪距离设置指令
   *
   */
  ros::Subscriber _sub1_, _sub2_, _sub3_, _sub4_;
  /**
   * @brief pub1_用于发布操作反馈,pub2_用于发布相机指令,_pub3_用于取消轨迹执行,_pub4_用于发布夹爪距离执行结果
   *
   */
  ros::Publisher _pub1_, _pub2_, _pub3_,_pub4_;
  /**
   * @brief
...
public:
  ...
  void exec_Callback(
    ...
    /**
   \brief 订阅grip_operation_out的回调函数,设置夹爪间距
   */
  void pgi_Callback(const std_msgs::StringConstPtr &pgiOperation_in);
  ...
  bool query_Callback(arm_eye::DmsService::Request &req,
