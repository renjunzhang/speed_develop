...
class RobotMsgManager:
    ...
    def __init__(self):
        ...
        # 空闲自动释放相关
        self.idle_since_ts = None          # 第一次变为IDLE的时间戳
        self.last_free_ts = 0              # 上次成功触发 freeRobot 的时间戳（秒）
        self.auto_free_cooldown = 1200     #冷却时间 
            ...
            try:
                ...
                for idx, operations in enumerate(self.DMS_msg["param"]):
                    ...
                    if cur_destination == self.now_workstation or cur_destination is None:
                        ...
                        self.oper_msg_gen(cur_destination, cur_operation)
                        self.OperAction(self.oper_msg, generate_msg_flag=False) #todo True
    ...
    def PubFreeRobot(self):
        jreq = self.agvlist.FreeRobotTask()
        try:
            resp = requests.post(
            ...
            logger.info(f"调用 freeRobot 接口异常: {e}")
            return False, None
        ...
        if dresp.get("code") == "0":
            ...
            logger.info("释放机器人失败: " + dresp.get("message", "未知错误"))
            return False, dresp
        

    # 发布充电任务
    def PubChargeTask(self):
        jreq = self.agvlist.ChargeTask()   
        try:
            resp = requests.post(
            ...
            logger.info(f"调用 agvChargeTask 接口异常: {e}")
            return False, None
        
        try:
            dresp = json.loads(resp.text)
        except Exception:
            logger.info(f"agvChargeTask 返回不可解析: {resp.text}")
            return False, None

        if str(dresp.get("code")) == "0":
            logger.info("充电任务发布成功")
            return True, dresp
        else:
            logger.info("充电任务发布失败: " + dresp.get("message", "未知错误"))
            return False, dresp 
    ...
    def update_status(self):  # 5秒一次的心跳监听
        ...
        while not rospy.is_shutdown():
            ...
            if cnt > 10:
                ...
                # ====== 在这里加空闲看门狗 ======
                now_sec = time.time()
                status = self.robot_heartbeat_msg["status"]

                # 若是充电/忙碌/错误，清空 idle 起点
                if status in ("CHARGING", "BUSY", "ERROR") or self.callback_fun_mutex.locked():
                    self.idle_since_ts = None

                # 进入或保持 IDLE：记录或维持起点时间
                elif status == "IDLE":
                    if self.idle_since_ts is None:
                        self.idle_since_ts = now_sec
                    else:
                        idle_duration = now_sec - self.idle_since_ts
                        # 满足：连续IDLE≥600s + 冷却期已过 + 不在回调互斥中
                        if idle_duration >= 600 and (now_sec - self.last_free_ts) >= self.auto_free_cooldown and not self.callback_fun_mutex.locked():
                            logger.info(f"IDLE 已持续 {int(idle_duration)} s，触发自动释放 freeRobot")
                            ok, dresp = self.PubFreeRobot()
                            # 不论成功失败，都记录一次触发时间，避免频繁请求
                            self.last_free_ts = now_sec
                            # 若成功释放，可选择清空 idle_since_ts，重新计时
                            if ok:
                                self.idle_since_ts = None
                                # 释放成功后触发充电任务（避免已在充电时重复触发）
                                # if self.robot_heartbeat_msg.get("status") != "CHARGING":
                                #     logger.info("释放成功，发布充电任务 agvChargeTask")
                                #     charge_ok, charge_resp = self.PubChargeTask()
                                #     logger.info(f"agvChargeTask 返回: {charge_resp}")
                # ====== 空闲看门狗结束 ======
